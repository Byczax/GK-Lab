...
def compile_shaders():
    vertex_shader_source = """
        ...
        float rand(in vec2 xy){ // gold noise
            return fract(tan(distance(xy*PHI,vec2(gl_VertexID, gl_InstanceID)* xy)*xy.y)*xy.x);
        }
        
        void main(void) {
            
            gl_Position = P_matrix * V_matrix * M_matrix * 
            (position + vec4(gl_InstanceID % 10 + rand(position.xy), gl_InstanceID / 10 - rand(position.xy), 0, 0));
            vertex_color = color;
        }
